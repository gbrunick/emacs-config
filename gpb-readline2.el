...
(defun gpb-make-readline (name program &optional startfile &rest switches)
  "Make a readline comint process NAME in a buffer, running PROGRAM.
...
Returns the (possibly newly created) process buffer."
  ...
  ;; enabled so we run stty before we exec the command.
  (setq gpb-comint-columns (window-width))
  (let ((buffer (make-comint name "sh" startfile "-c"
                             (mapconcat 'identity
                                        `("stty echo;" "TERM=vt100;"
                                          "export TERM;" "exec"
                                          ,program ,@switches)
                                        " "))))
    (with-current-buffer buffer (readline-mode 1))
    buffer))
  ...
  (let ((proc (get-buffer-process (current-buffer)))
    ...
    (cond
     ...
     (t
      ...
      (with-current-buffer (get-buffer-create "*readline*")
        (erase-buffer))
      (let* ((proc (get-buffer-process (current-buffer)))
             (redirect-buffer (get-buffer-create "*readline*"))
             (prompt-regexp (mapconcat
                            'identity
                            (mapcar 'regexp-quote
                                    (split-string (gpb-comint:current-prompt)
                                                  "" 'omit-nulls))
                            (rx (0+ (or "\C-m"
                                        (regex "\e\\[[0-9;]*m")
                                        "]0;.*"))))))
        ...
        (comint-redirect-setup redirect-buffer (current-buffer)
                               prompt-regexp nil)
        (let ((lexical-binding t))
          (add-function :around (process-filter proc) #'comint-redirect-filter))
        ;; (display-buffer buffer)
        ;; (comint-send-string proc (concat command "<readline-complete>\C-U\C-K"))

        ;; (comint-redirect-send-command-to-process
        ;;  (concat command "<readline-complete>\C-U\C-K")
        ;;  "*readline*" proc nil)

      ;; (setq-local comint-redirect-finished-regexp (gpb-comint:current-prompt))
        (message "comint-redirect-finished-regexp %S"
...
(defun gpb-rl-redirect-filter (str)
...
`pop-to-buffer')."
  ...
  (let ((buffer ;; (if arg
    ...
    (if (comint-check-proc buffer)
      ...
      (goto-char (point-max))
      (insert "Running bash...\n")
      ;;(gpb-make-readline "bash")
    (let ((proc (get-buffer-process (gpb-make-readline "bash" "bash"))))
      (set-process-sentinel proc 'gpb-bury-buffer-on-process-exit)
